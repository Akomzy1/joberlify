...
**The fundamental shift:** Traditional job sites feature jobs and hope you find the right one. Joberlify features YOU and goes out to find the right job. It learns your skills, experience, career direction, and visa needs — then proactively scans company career pages, filters out irrelevant roles, and surfaces only the opportunities where you have a genuine match. Every listing you see has already been scored against your profile.
...
The core philosophy is **quality over quantity**. Joberlify tells users when NOT to apply ("Not Yet") and gives them actionable guidance to close gaps — turning rejection into a growth moment. It is NOT a spray-and-pray auto-apply tool. It is NOT a traditional job board. It is a personal recruitment agent that does the searching, screening, scoring, and prep work — so the user only makes the final call.
...
- **Location scoring logic:** location_fit checks the job location against the user's target_locations (specific cities/regions per country), remote_preference, and max_commute_miles:
  - Job in user's specified city → 5.0
  - Job is remote + user prefers remote or open → 5.0
  - Job in user's target country but different city + user is open to relocating → 3.5
  - Job in user's target country but different city + user NOT open to relocating → 2.0
  - Job in a country NOT in user's target_countries → 1.0
  - Job is remote + user is onsite_only → 2.0
  - If location_fit < 2.0, flag in gap report: "This role is in [location]. Your target locations are [user_locations]."
...
- `user_profiles` — nationality, target countries, target locations (city/region level), remote preference (onsite/hybrid/remote/open), max commute distance, visa needs, parsed CV data, skills, qualifications
